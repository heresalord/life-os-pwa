...
export function GoalItem({ goal, progress, date }: { goal: Goal, progress: number, date: string }) {
  ...
  // Label shows measurement type as unit hint
  const unitLabel = goal.measurement_type === 'currency' && goal.currency
    ? goal.currency
    : goal.measurement_type === 'time'
    ? 'hrs'
  ...
  return (
    <div className="bg-surface border border-border rounded-2xl p-4 flex flex-col gap-3">
      ...
      <div className="h-2 bg-surface-2 rounded-full overflow-hidden">
        <div
          className={`h-full rounded-full transition-all duration-500 ${isComplete ? 'bg-success' : 'bg-accent/70'}`}
          style={{ width: `${pct}%` }}
        />
      </div>
      ...
      {isComplete && (
        <div className="text-xs text-success font-medium flex items-center justify-between">
          <span>Goal met! 🎉</span>
          <button onClick={handleArchive} className="text-text-muted hover:text-text transition-colors">
            Archive
          </button>
        </div>
      )}
