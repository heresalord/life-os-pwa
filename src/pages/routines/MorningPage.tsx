...
export function MorningPage() {
  ...
  useEffect(() => {
    ...
    if (step === 1 && carrying) runCarryOver()
        ...
        <div className="space-y-8 animate-in fade-in slide-in-from-right-4 duration-500">
          <div className="text-center space-y-4">
            ...
            <p className="text-text-secondary leading-relaxed">
              {carrying ? 'Reviewing pending tasks from yesterday...'
                : carryCount === 0 ? 'You had a clean slate yesterday! Nothing to carry over.'
                : `Moved ${carryCount} pending task${carryCount !== 1 ? 's' : ''} to today's list.`}
            </p>
          </div>
          <button onClick={() => setStep(2)} disabled={carrying}
            className="w-full flex items-center justify-center gap-2 py-3.5 bg-warning text-bg font-medium rounded-xl hover:bg-warning/90 transition-colors disabled:opacity-50">
        ...
        <div className="space-y-8 animate-in fade-in slide-in-from-right-4 duration-500">
          <div className="space-y-4">
            ...
            <p className="text-text-secondary leading-relaxed text-sm">
              What is the single most important thing for you today?
            </p>
            <textarea autoFocus value={intent} onChange={e => setIntent(e.target.value)}
              placeholder="Today, I intend to..." rows={4}
          ...
          <div className="flex gap-3">
            ...
            <button onClick={handleNextStep2}
              className="flex-[2] flex items-center justify-center gap-2 py-3.5 bg-warning text-bg font-medium rounded-xl hover:bg-warning/90 transition-colors">
              Continue <ArrowRight size={18} />
            </button>
          </div>
        </div>
      ...
      {step === 3 && (
        <div className="space-y-8 animate-in fade-in slide-in-from-right-4 duration-500">
              ...
              <div className="space-y-2 max-h-[40vh] overflow-y-auto pr-2">
                {agenda.sort((a, b) => a.start_time.localeCompare(b.start_time)).map(b => (
                  <div key={b.id} className="flex items-center gap-3 p-3 bg-surface-2 rounded-xl border border-border text-sm">
                    <div className="w-16 flex-shrink-0 text-text-muted font-medium">{b.start_time.slice(0, 5)}</div>
                    <div className="w-px h-8 bg-border" />
                    <div className="truncate text-text font-medium">{b.description}</div>
                  </div>
                ))}
              </div>
            )}
          ...
          <div className="flex gap-3 pt-4 border-t border-border/50">
            ...
            <button onClick={() => navigate('/')} className="flex-[2] py-3.5 bg-warning text-bg font-medium rounded-xl hover:bg-warning/90 transition-colors">
              Start Day
            </button>
          </div>
        </div>
