...
export function ReviewPage() {
  ...
  const handleFinish = async () => {
    ...
    // Save reflection note if written
    if (reflection.trim()) {
              ...
              {MOODS.map(m => (
                <button
                  ...
                  className={`flex flex-col items-center gap-3 p-3 rounded-2xl transition-all ${
                    mood === m.value 
                      ? 'bg-info/20 scale-110 shadow-lg shadow-info/10' 
                      : 'hover:bg-surface-2 grayscale hover:grayscale-0'
                  }`}
                >
                  <span className="text-4xl">{m.emoji}</span>
                  <span className={`text-[10px] font-medium uppercase tracking-wider ${mood === m.value ? 'text-info' : 'text-text-muted'}`}>
                    {m.label}
                  </span>
                </button>
              ))}
      ...
      {step === 2 && (
        <div className="space-y-6 flex flex-col h-full animate-in fade-in slide-in-from-right-4 duration-500">
          <div>
            ...
            <p className="text-sm text-text-secondary">Mark pending tasks as done, skip them, or leave them to automatically carry over tomorrow.</p>
          </div>
          ...
          <div className="flex-1 overflow-y-auto max-h-[50vh] space-y-2 pr-2">
            {pendingTasks.length === 0 ? (
              <div className="p-8 text-center bg-surface-2 rounded-2xl border border-border border-dashed">
                ...
                <p className="text-xs text-text-muted mt-1">Great job clearing your list.</p>
              </div>
            ) : (
              pendingTasks.map(t => (
                <TaskItem 
                  ...
                  onDelete={() => {}} // Not rendering delete in review flow typically, but keeping prop satisfied
                />
              ))
            )}
          </div>
      ...
      {step === 3 && (
        <div className="space-y-6 flex flex-col h-full animate-in fade-in slide-in-from-right-4 duration-500">
          <div>
            ...
            <p className="text-sm text-text-secondary">Process any remaining thoughts so you can disconnect with a clear mind.</p>
          </div>
