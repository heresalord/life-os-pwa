...
export function OverviewTab({ currency }: { currency: string }) {
  ...
  const chartData = useMemo(() => {
    const days = eachDayOfInterval({
      start: new Date(monthFrom + 'T12:00:00'),
  ...
  return (
    <div className="space-y-6 animate-in fade-in slide-in-from-bottom-2 duration-300">
      ...
      <div className="bg-gradient-to-br from-surface to-surface-2 border border-border rounded-2xl p-6">
        ...
        <p className="text-4xl font-display font-medium text-text">{netBalance.toFixed(2)} <span className="text-xl text-text-muted">{currency}</span></p>
        {wallets.length > 0 && (
          <div className="flex gap-2 mt-4 overflow-x-auto pb-2 scrollbar-none">
            {wallets.map((w: Wallet) => (
              <div key={w.id} className="flex-shrink-0 bg-surface border border-border rounded-lg px-3 py-2 flex items-center gap-3">
                <div className="w-3 h-3 rounded-full" style={{ backgroundColor: w.color || '#4ade80' }} />
                <div>
                  <p className="text-[10px] text-text-muted uppercase tracking-wide">{w.name}</p>
                  <p className="text-sm font-medium">{w.balance.toFixed(2)} {w.currency}</p>
                </div>
              </div>
            ))}
          </div>
      ...
      <div className="grid grid-cols-2 lg:grid-cols-3 gap-4">
        <div className="bg-surface border border-border rounded-2xl p-5">
          <div className="flex items-center gap-2 mb-3">
            ...
            <span className="text-xs text-text-muted uppercase tracking-wider">Spent (30d)</span>
          </div>
        ...
        <div className="bg-surface border border-border rounded-2xl p-5">
          <div className="flex items-center gap-2 mb-3">
            ...
            <span className="text-xs text-text-muted uppercase tracking-wider">Earned (30d)</span>
          </div>
        ...
        <div className="hidden lg:block bg-surface border border-border rounded-2xl p-5">
          <div className="flex items-center gap-2 mb-3">
            <WalletIcon size={16} className={over ? 'text-warning' : 'text-text-muted'} />
            <span className="text-xs text-text-muted uppercase tracking-wider">Net (30d)</span>
          </div>
          ...
          <div className="mt-2">
            <div className="flex justify-between text-xs text-text-muted mb-1">
              <span>Global Budget {Math.round(budgetPct)}%</span>
