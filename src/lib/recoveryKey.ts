// ── Master Recovery Key Utilities (BIP-39 standard wordlist) ─────────────────

// 1024 curated BIP-39 English words
...
/**
 * Generate a 12-word recovery phrase using cryptographically secure random values.
 */
...
/**
 * Normalize and calculate SHA-256 hash of the recovery phrase combined with the user's email.
 */
export async function hashRecoveryPhrase(phrase: string | string[], email: string): Promise<string> {
  const words = Array.isArray(phrase) ? phrase : phrase.trim().split(/\s+/)
  const normalizedPhrase = words.map(w => w.toLowerCase().trim()).join(' ')
  const normalizedEmail = email.toLowerCase().trim()
  const payload = `${normalizedPhrase}:${normalizedEmail}`
 ...
 * Storage key helper
 */
const STORAGE_PREFIX = 'life_os_recovery_'
...
/**
 * Get or create the recovery key for the current user.
 */
export function getOrCreateUserRecoveryKey(userId: string): RecoveryKeyStatus {
  const key = `${STORAGE_PREFIX}${userId}`
  const raw = localStorage.getItem(key)
  if (raw) {
    try {
      return JSON.parse(raw) as RecoveryKeyStatus
    } catch {
      // ignore
    }
  }
  ...
  const newStatus: RecoveryKeyStatus = {
    phrase,
    verified: false,
...
/**
 * Mark user's recovery key as verified and save the cryptographic hash.
 */
export async function markUserRecoveryKeyVerified(userId: string, email: string): Promise<string> {
  const key = `${STORAGE_PREFIX}${userId}`
  const status = getOrCreateUserRecoveryKey(userId)
  status.verified = true
  const hash = await hashRecoveryPhrase(status.phrase, email)
  status.hash = hash
  localStorage.setItem(key, JSON.stringify(status))

  // Also save a global map for offline recovery by email lookup on this device
  try {
    const emailKey = `life_os_recovery_hash_${email.toLowerCase().trim()}`
    localStorage.setItem(emailKey, JSON.stringify({ userId, hash, verifiedAt: new Date().toISOString() }))
  } catch {
    // ignore
  }
...
/**
 * Check if the user has backed up and verified their recovery key.
 */
export function isUserRecoveryKeyVerified(userId?: string | null): boolean {
  if (!userId) return true
  const key = `${STORAGE_PREFIX}${userId}`
  const raw = localStorage.getItem(key)
  if (!raw) return false
  try {
    const parsed = JSON.parse(raw) as RecoveryKeyStatus
